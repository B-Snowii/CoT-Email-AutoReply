...
def build_few_shot_prompt(message, temperature=0.3, top_p=0.9, top_k=25):
    # Add system message for generation parameters
    system_message = (
        "<|system|>\n"
        f"top_k={top_k}\n"
...
# --- Llama 2 local (Ollama) subprocess call ---
def get_llama2_response(prompt, model_name="llama2"):
    import subprocess
...
def generate_answers(message, temperature=0.3, top_p=0.9, top_k=25):
    prompt = build_few_shot_prompt(message, temperature, top_p, top_k)
    output = get_llama2_response(prompt, model_name="llama2")
    return output
